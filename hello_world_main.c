...
// Tag para logs
static const char *TAG = "Evelyn-RM:88055";
...
void init_task_watchdog(void)
{
    ...
    printf("{Evelyn-RM:88055} [WDT] Watchdog inicializado com timeout de %d segundos.\n", WDT_TIMEOUT_S);
}
...
void task_generate(void *pvParameters)
{
    ...
    while (1)
    {
        ...
        if (sent == pdPASS)
        {
            printf("{Evelyn-RM:88055} [FILA] Dado %d enviado com sucesso!\n", value);
        }
        else
        {
            printf("{Evelyn-RM:88055} [FILA] Fila cheia, dado %d descartado!\n", value);
        }
...
void task_receive(void *pvParameters)
{
    ...
    while (1)
    {
        if (xQueueReceive(dataQueue, &receivedValue, pdMS_TO_TICKS(RECEPTION_TIMEOUT_MS)) == pdPASS)
        {
            ...
            {
                *ptr = receivedValue;
                printf("{Evelyn-RM:88055} [RECEPÇÃO] Valor recebido: %d\n", *ptr);
                free(ptr);
            }
            else
            {
                printf("{Evelyn-RM:88055} [RECEPÇÃO] Erro ao alocar memória!\n");
            }
        ...
        else
        {
            ...
            printf("{Evelyn-RM:88055} [RECEPÇÃO] Nenhum dado recebido no tempo limite!\n");
            printf("{Evelyn-RM:88055} [RECEPÇÃO] Tentando recuperar...\n");
            ...
            {
                printf("{Evelyn-RM:88055} [RECEPÇÃO] Recuperação bem-sucedida! Valor: %d\n", receivedValue);
                reception_ok = true;
            }
            else
            {
                printf("{Evelyn-RM:88055} [RECEPÇÃO] Falha persistente. Encerrando tarefa...\n");
                reception_ok = false;
                vTaskDelete(NULL);
            }
        }
...
void task_supervision(void *pvParameters)
{
    ...
    while (1)
    {
        printf("\n{Evelyn-RM:88055} [SUPERVISÃO] STATUS DO SISTEMA:\n");
...
void app_main(void)
{
    printf("{Evelyn-RM:88055} [SISTEMA] Inicializando sistema multitarefa...\n");
    ...
    if (dataQueue == NULL)
    {
        printf("{Evelyn-RM:88055} [ERRO] Falha ao criar fila!\n");
        return;
    }
    ...
    printf("{Evelyn-RM:88055} [SISTEMA] Tarefas iniciadas com sucesso!\n");
}
